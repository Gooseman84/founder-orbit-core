...
serve(async (req) => {
  ...
  try {
    ...
    // ── Build System Prompt ───────────────────────────────────
    // ── Inject Execution Strategy (Feedback Loop) ──────────────
    const strategy = executionStrategy?.strategy as any;
    const strategyBlock = strategy ? `
## EXECUTION STRATEGY (from yesterday's feedback loop analysis)
- Primary Focus: ${strategy.primary_focus || "none set"}
- Recommended Task Count: ${strategy.task_count_recommendation || "2-3"}
- Max Task Duration: ${strategy.max_task_minutes || 120} minutes
- Prioritize Categories: ${(strategy.prioritize_categories || []).join(", ") || "any"}
- Avoid Categories: ${(strategy.avoid_categories || []).join(", ") || "none"}
- Energy Calibration: ${strategy.energy_calibration || "normal"}
${(strategy.directives || []).length > 0 ? `- Directives:\n${strategy.directives.map((d: string) => `  • ${d}`).join("\n")}` : ""}
${(strategy.active_pattern_warnings || []).length > 0 ? `- Pattern Warnings:\n${strategy.active_pattern_warnings.map((w: string) => `  ⚠️ ${w}`).join("\n")}` : ""}
${(strategy.market_gaps || []).length > 0 ? `- Market Gaps to Address:\n${strategy.market_gaps.map((g: string) => `  📊 ${g}`).join("\n")}` : ""}

IMPORTANT: Use this strategy to calibrate task count, difficulty, and focus. This strategy was generated by analyzing the founder's behavioral patterns, completion data, and market validation results.
` : "";

    const taskCount = append ? "1-2" : (strategy?.task_count_recommendation ? `${strategy.task_count_recommendation}` : "1-3");
